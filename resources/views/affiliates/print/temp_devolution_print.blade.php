   ...
   <div id="project">
    <table class="table" style="width:100%;">
      ...
      <tr><td class="text-justify">
        ...
        <?php $ii=sizeOf($devolution->dues()->whereIn('eco_com_procedure_id', [9,10,11,12])->where('amount','>',0)->get());?>
        @foreach($devolution->dues()->whereIn('eco_com_procedure_id', [9,10,11,12])->where('amount','>',0)->get() as $index => $d)
          @if($index<$ii-1)
        ...
        <strong>
        expreso mi conformidad de manera voluntaria para que se efectúe el descuento del beneficio del Complemento Económico, hasta cubrir el monto determinado.
      ...
      @include('affiliates.print.devolutions.amount', ['dues'=>$devolution->dues()->whereIn('eco_com_procedure_id', [9,10,11,12])->get(),'devolution'=>$devolution,'total_dues_literal'=>$total_dues_literal, 'total'=>$total])
