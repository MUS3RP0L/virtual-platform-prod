...
class ExportExcel extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("Ejecutando Exportacion");
           ...
           //  $this->info(" mes ".$mes);
             // exit();
           $path = storage_path('excel/imports/fechas_disponibilidad.xlsx');
           ...
           global $rows,$afiliadoConflicto;
            ...
            //*individual*/
                    ...
                    foreach ($sheet as $r) 
                    {
                        ...
                        if($afiliado )
                        {
                                if($r->fecha_disponibilidad)
                                {
                                    ...
                                    $time = strtotime($r->fecha_disponibilidad);

                                    $fecha = date('Y-m-d',$time);
                                    Log::info("fecha: ".$fecha);

                                    $contribuciones_c =  Contribution::where('affiliate_id','=',$afiliado->id)->where('breakdown_id','=',1)->where('month_year','>',$fecha)->get();
                                    if($contribuciones_c)
                                    {
                                    ...
                                    $monto_total +=$monto_contribuciones;
                    ...
                    array_push($rows, array('','','','','','Total Bs:',$monto_total,'',''));
