    ...
    public function handle()
    {
        ...
        switch ($sw) {
            ...
            case 'p':
                ...
                //     {
                //        Log::info($afiliado->identity_card." ".$afiliado->date_entry);    
                //        array_push($rows, array($afiliado->identity_card,"Observacion por Presatamos"));
            ...
            case 'r':
                ...
                global $Progress,$rows,$no_registrados;
                 ...
                 foreach ($hoja as $fila) {
                    ...
                    // Log::info(json_encode($applicant));


                    // if($applicant)
                    // {
                    //    Log::info($applicant->identity_card." Encontrado el HDP");    
                    //    array_push($rows, array($applicant->identity_card,"Observacion por Reposicion de Fondos"));

                       
                    //    // $observacion->save();
                    //    Log::info("buscando:".$fila->tipo);
                    //    $modalidad = DB::table("eco_com_modalities")->where("shortened","=",$fila->tipo)->first();
                    //    Log::info("id modalida ->".$modalidad->id);
                    //    Log::info("economic_id =".$applicant->economic_complement_id);
                    //    $complemento = EconomicComplement::find($applicant->economic_complement_id);
                    //    Log::info($complemento);

                    //    $tramite = EconomicComplement::where('id','=',$applicant->economic_complement_id)
                    //                                 ->where('eco_com_modality_id','=',$modalidad->id)
                    //                                 ->where("eco_com_procedure_id","=","2")->first();
                    //    Log::info("tramite hdp");                                                
                    //    Log::info($tramite);                            
                    //    // $observacion = new AffiliateObservation;
                    //    // $observacion->user_id = 1;//user alejandro XD
                    //    // $observacion->affiliate_id= $tramite->affiliate_id;
