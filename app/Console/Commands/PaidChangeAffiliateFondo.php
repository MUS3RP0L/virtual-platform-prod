...
use DB;
class PaidChangeAffiliateFondo extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:paid_affiliate_fondo';
    ...
    public function handle()
    {
        ...
        if ($password == ACCESS) {
            ...
            if ($this->confirm('Are you sure to import the folder "' . $FolderName . '" ? [y|N]') && $FolderName) {
                ...
                Excel::batch('public/file_to_import/' . $FolderName . '/', function($rows, $file) {
                    $rows->each(function($result) {
                        ...
                        if ($aff) {
                            ...
                            $exists=DB::table('paid_affiliates')->where('type','=','F')->where('affiliate_id', '=', $aff->id)->first();
                            if (!$exists) {
                ...
                Excel::create('Lista Afiliados no encontrados'.date("Y-m-d H:i:s"),function($excel)
                {
