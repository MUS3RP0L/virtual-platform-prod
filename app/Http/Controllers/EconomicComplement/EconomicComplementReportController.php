...
class EconomicComplementReportController extends Controller
{
   ...
   public function data_shet($id_complemento)
    ...
    public function print_total($eco_com_id)
    {
        ...
        if ($economic_complement->old_eco_com) {
        $old_eco_com_total_calificate = $old_eco_com->total + ($old_eco_com->amount_loan ?? 0) + ($old_eco_com->amount_accounting ?? 0) + ($old_eco_com->amount_replacement ?? 0)+ ($old_eco_com->amount_credit ?? 0);
        }
        $data = [
...
foreach ($doble1 as $a) {
    ...
    $i++;
        ...
        break;
        //###########################################################################################################################
        //PAGADOS EN BANCO UNION DISGREGADOS POR AMORTIZACIONES
        case '28' :
        // AFFILIADOS SIN OBSERVACION
            $columns = '';
            $file_name = $name.' '.date("Y-m-d H:i:s");
...
foreach ($doble1 as $a) {
    ...
    $i++;
        ...
        Excel::create('Pagados en banco' . date("Y-m-d H:i:s"), function ($excel) 
        {
        ...
        //###########################################################################################################################
        //REZAGADOS EN BANCO UNION DISGREGADOS POR AMORTIZACIONES
        case '29' :
        // AFFILIADOS SIN OBSERVACION
            $columns = '';
            $file_name = $name.' '.date("Y-m-d H:i:s");
...
foreach ($doble1 as $a) {
    ...
    $i++;
        ...
        Excel::create('Rezagados de Banco' . date("Y-m-d H:i:s"), function ($excel) 
        {
